...
export class AppointmentService {
  ...
  private canManage(existing: { createdById: string; directorId: string }, user: AuthUser) {
    if (user.role === 'ADMIN') return true;
    if (existing.createdById === user.id) return true;
    if (existing.directorId === user.id) return true; // le directeur peut gérer son propre agenda
    return false;
  }
  ...
  async update(id: string, data: UpdateAppointmentInput, user: AuthUser) {
    const existing = await this.repo.findById(id);
    if (!existing) throw new AppError('Rendez-vous introuvable', 404);
    if (!this.canManage(existing, user)) throw new AppError('Non autorisé', 403);
    ...
    if (data.status && data.status !== oldStatus) {
      ...
      if (fullAppointment) {
        const updater = await prisma.user.findUnique({ where: { id: user.id } });
        this.notificationService.notifyStatusChange(
          fullAppointment as any,
          oldStatus,
          updater!
