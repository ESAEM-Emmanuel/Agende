  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <header className="bg-blue-900 text-white p-4 flex justify-between items-center flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold">Statistiques & Analyses</h1>
          <p className="text-sm opacity-80">Tableau de bord analytique</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => router.push('/admin/users')} className="text-sm opacity-80 hover:opacity-100 px-3 py-2">
            ← Utilisateurs
          </button>
          <button onClick={() => router.push('/dashboard')} className="text-sm opacity-80 hover:opacity-100 px-3 py-2">
            Agenda
          </button>
        </div>
      </header>
      ...
      <main className="p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 flex flex-wrap gap-4 items-center">
          <div>
            ...
            <select value={year} onChange={(e) => setYear(parseInt(e.target.value))} className="px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none">
          ...
          <div>
            ...
            <select value={directorId} onChange={(e) => setDirectorId(e.target.value)} className="px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none min-w-[200px]">
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
              {[
                ...
                <div key={card.label} className="bg-white p-4 rounded-xl border border-slate-200 text-center">
                  <div className={`text-2xl font-bold ${card.color}`}>{card.value}</div>
                  <div className="text-xs text-slate-500 uppercase font-bold mt-1">{card.label}</div>
                </div>
              ))}
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-xl border border-slate-200">
                <h3 className="text-lg font-bold text-slate-800 mb-4">Par directeur</h3>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={stats.byDirector} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                      <XAxis type="number" tick={{ fontSize: 12 }} />
                      <YAxis dataKey="directorName" type="category" width={120} tick={{ fontSize: 11 }} />
                      <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} />
                      <Legend />
            ...
            <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
              <div className="p-4 border-b border-slate-200">
                <h3 className="text-lg font-bold text-slate-800">Top Acteurs (Participants)</h3>
                <p className="text-sm text-slate-500">Les personnes les plus sollicitées dans les rendez-vous</p>
              </div>
