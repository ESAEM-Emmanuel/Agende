    ...
    <div className="flex gap-4">
      <div className="w-16 flex-shrink-0">
        {Array.from({ length: 14 }, (_, i) => i + 7).map(h => (
          <div key={h} className="h-16 border-b border-slate-100 text-right pr-2 pt-1 text-xs text-slate-400">{h}h</div>
        ))}
      </div>
      <div className="flex-1 relative">
        ...
        {dayAppts.map(appt => {
          ...
          const height = ((endH - startH) * 60 + (endM - startM)) / 60 * 64 - 2;
          ...
          return (
            ...
            >
              ...
              {appt.status === 'PENDING' && (
                <span className="inline-block mt-1 px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] rounded font-bold">En attente</span>
              )}
              {appt.status === 'DONE' && (
                <span className="inline-block mt-1 px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] rounded font-bold">✓ Terminé</span>
              )}
              {appt.priority === 'HIGH' && appt.status !== 'DONE' && (
                <span className="inline-block mt-1 ml-1 px-2 py-0.5 bg-red-100 text-red-700 text-[10px] rounded font-bold">Prioritaire</span>
              )}
            </div>
          );
        })}
